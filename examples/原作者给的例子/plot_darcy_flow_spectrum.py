...
演示了如何使用傅立叶变换进行小规模达西流(Darcy-Flow)数据集的频谱分析。
主要目的是分析流体流动数据集的能量分布，以获取对湍流或其他数据集行为及其基础物理过程的洞察。
Short summary
-------------
...
#为了结果可复现性，设置了 PyTorch 和 NumPy 的随机种子，并调整了 Matplotlib 字体的大小。
font = {'size'   : 28}
...
# Shape of the dataset检查原始数据集的形状，挤压掉不必要的维度，最终得到 dataset_pred
shape = dataset_pred.shape

# 定义数据集的网格- 为了进行频谱分析，定义了数据集的二维网格
# Example for 3d grid
"""
...
##############################################################
#频谱分析和绘图
#利用 spectrum_2d 函数对数据集进行二维傅立叶变换，生成频谱数据。
#利用 Matplotlib 绘制了频谱图，设置了一些绘图参数，如对数刻度等。
...
linewidth = 3
ax.set_yscale('log')
#ax.set_ylim(10, 1e10)  # 使用科学计数法表示1e10，确保为正数
