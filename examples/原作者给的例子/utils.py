...
def l2_outlier(y_true, y_pred):
    error = np.ravel((y_true - y_pred) ** 2)
    u = np.ravel(y_true**2)
    error = np.sort(error)[: -len(error) // 1000]
    u = np.sort(u)[: -len(u) // 1000]
    error_sum = np.sum(error)
    u_sum = np.sum(u)
    return np.sqrt(error_sum)/np.sqrt(u_sum)
...
def safe_test(model, data, X_test, y_test, fname=None):
    ...
    l2_error = l2_outlier(y_test, y_pred)
    print("Test MSE w/o outliers: {}\n".format(error))
    print("Test l2 w/o outliers: {}\n".format(l2_error))
    if fname is not None:
        np.savetxt(fname, np.hstack((X_test[1], y_test, y_pred)))

def safe_hahaha(model, X_test, fname=None):
    def is_nonempty(X):
        return len(X[0]) > 0 if isinstance(X, (list, tuple)) else len(X) > 0
